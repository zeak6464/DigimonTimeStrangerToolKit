...
function Update(elapsed_frame)
  ...
  if player_state == "near" then
    if self_state ~= "make" then
      Self().Motion_PlayMotionAndSetting(MOTION, 0, gEvalFrame, MOTION_START_FRAME, MOTION_END_FRAME, 5, false)
      SetExternalVariable("state", "make")
    end
    gEvalFrame = Self().GetMotionEvalFrame(MOTION)
  end
  if player_state == "far" then
    if self_state ~= "broke" then
      Self().Motion_PlayMotionAndSetting(MOTION, 0, gEvalFrame, MOTION_END_FRAME, MOTION_START_FRAME, 5, false)
      SetExternalVariable("state", "broke")
    end
    gEvalFrame = Self().GetMotionEvalFrame(MOTION)
  end
end
