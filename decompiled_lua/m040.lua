...
function M040:Event_070()
  if not Flg.Check("FLAG_MAIN_02_060") and Qst.Main:Check(self.ID, self.STEP_025, self.STEP_255) then
    FadeOutWithWaitNotFlgCheck(FADE_BLACK, FADE_TIME)
    Execute("m040_060")
    Guest.Add(15631)
    Flg.Set("FLAG_SKIP_PLAYBGM_MAPCHANGE")
    Fade.Out(FADE_WHITE, 0)
    Battle.Encount(M040.Battle.Enemy.e01, M040.Battle.Map.m01, false)
  end
end

function M040:Event_080()
  if Flg.Check("FLAG_MAIN_02_060") and Qst.Main:Check(self.ID, self.STEP_025, self.STEP_255) then
    Execute("m040_100")
    Common.SetDigimonGraspState(564, DIGIMON_GRASP_FLAG_SCAN)
    Guest.Delete(15631)
    Flg.Set("FLAG_SKIP_PLAYBGM_MAPCHANGE")
    MapChange("t", 301, "start_03", false, 0, FADE_TIME)
  end
end

function M040:Event_090()
  if not Flg.Check("FLAG_MAIN_02_070") and Qst.Main:Check(self.ID, self.STEP_025, self.STEP_255) then
    Execute("m040_110")
    Execute("m040_130")
    Guest.Delete(30021)
    Flg.Set("FLAG_MAIN_02_070")
    Qst.Main:End(self.ID, self.STEP_255)
    Restore.Party()
    MapChangeWithTimeTravel("t", 404, "start_01", AXIS_ORIGINAL, "A2020_dbef_50", NOON)
  end
end
