...
function Start()
  ...
  root_os_pos_x, root_os_pos_y, root_os_pos_z = GetPos()
  state = 1
  ...
  for i = 1, 30 do
    rpy[i] = math.sin(math.pi * 2 / 30 * i) * DISTANCE
  end
  for i = 1, gic do
    instance[i] = {
...
function Update()
  local plr_x, plr_y, plr_z = GetPlayerPos()
  df = GetDeltaFrame(30)
  local count_i = math.floor(count) + start_ry_offset
  local cf = count_i % LOOPCYCLE + 1
  local cfy = count_i % (SHAKE_FRAME * df) + 1
