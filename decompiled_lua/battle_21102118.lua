...
function Battle_Command_Start(info)
  if Flg.Check("Battle5051") then
    Battle.SetNextCommandSkill(ENEMY_DIGIMON, 0, 27393)
...
function Battle_Turn_End(info)
  ...
  if Battle.IsAccomplishConditionType2(BATTLE_CONDITION_TYPE_DAMAGE_MAGNIFICATION_BELOW, 0, ENEMY_DIGIMON, 0, 4, 100, false) then
    Flg.Set("Battle5015")
  end
  if Battle.IsAccomplishCondition(BATTLE_CONDITION_TYPE_DAMAGE_MAGIC, 3, ENEMY_DIGIMON, 0, 6) then
    Flg.Set("Battle5016")
  end
  if Battle.IsAccomplishCondition(BATTLE_CONDITION_TYPE_DAMAGE_VALUE_OVER, 0, ENEMY_DIGIMON, 0, 3000) then
    Flg.Set("Battle5017")
  end
  if Battle.GetCharacterLastCommandSkillID(ENEMY_DIGIMON, 0) == 27393 then
...
function Battle_Ending(info)
  Common.SetDigimonGraspState(739, DIGIMON_GRASP_FLAG_SCAN)
end
