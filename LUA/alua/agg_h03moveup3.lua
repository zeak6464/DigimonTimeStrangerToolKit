...
local rrx, rry, rrz = {}, {}, {}
local count_frame
local loop_frame = 200
local start_fade_frame = 100
local r_spd = 0.075
...
function reset()
  for i = 1, GetInstanceCount() do
    cpx[i] = math.random(-20, 20) * 0.01
    cpy[i] = math.random(-150, 300) * 0.01
    cpz[i] = math.random(-20, 20) * 0.01
    crx[i] = 0
    cry[i] = 0
    crz[i] = 0
    rpx[i] = math.random(-20, 20) * 5.0E-4
    rpy[i] = math.random(50, 120) * 0.002
    rpz[i] = math.random(-20, 20) * 5.0E-4
    rrx[i] = math.random(-100, 100) * 0.01
    rry[i] = math.random(-100, 100) * 0.01
    rrz[i] = math.random(-100, 100) * 0.01
    csx[i] = math.random(1, 6)
    csy[i] = csx[i]
    csz[i] = csx[i]
  end
end
