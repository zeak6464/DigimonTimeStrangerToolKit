...
function Battle_Start(info)
  ...
  if 50 < enemy_level then
    ...
    Battle.UpgradeSkill(GUEST_DIGIMON, 1, 30312, 30313)
    Battle.UpgradeSkill(ENEMY_DIGIMON, 0, 30312, 30313)
    Battle.UpgradeSkill(ENEMY_DIGIMON, 0, 30042, 30043)
    ...
    Battle.UpgradeSkill(ENEMY_DIGIMON, 0, 29062, 29063)
  end
end
...
function Battle_Turn_Start(info)
  ...
  if Flg.Check("Battle5053") and info.group == 2 then
    if info.index == 0 then
      if Battle.IsBuff(ENEMY_DIGIMON, 1, 203100, BUFF_CHECK_AND_WITH_ID) == true then
        Battle.SetNextCommandTarget(GUEST_DIGIMON, 0, ENEMY_DIGIMON, 1)
      elseif Battle.IsBuff(ENEMY_DIGIMON, 2, 203100, BUFF_CHECK_AND_WITH_ID) == true then
        Battle.SetNextCommandTarget(GUEST_DIGIMON, 0, ENEMY_DIGIMON, 2)
      else
    ...
    elseif info.index == 1 then
      ...
      elseif Battle.IsBuff(ENEMY_DIGIMON, 2, 203100, BUFF_CHECK_AND_WITH_ID) == true then
        Battle.SetNextCommandTarget(GUEST_DIGIMON, 1, ENEMY_DIGIMON, 2)
      else
        Battle.SetNextCommandTarget(GUEST_DIGIMON, 1, ADD_TARGET, 0)
      end
      Battle.SetNextCommandSkill(GUEST_DIGIMON, 1, 30312)
    end
  end
...
function Battle_Command_Start(info)
  ...
  if Flg.Check("Battle5053") and Battle.GetCharacterLastCommandSkillID(ENEMY_DIGIMON, 0) ~= 70005 and info.group == 1 then
...
function Battle_Action_Start(info)
  ...
  if info.group == ENEMY_DIGIMON and info.enemy_id == 90601 then
    local skill_id = Battle.GetLastCommandSkillID()
    if skill_id == 29061 and info.use_count == 0 then
...
function Battle_Turn_End(info)
  ...
  if 0 < Battle.GetStatus(ENEMY_DIGIMON, 1, STATUS_TYPE_HP) or 0 < Battle.GetStatus(ENEMY_DIGIMON, 2, STATUS_TYPE_HP) then
    Flg.Set("Battle5013")
  end
  ...
  if stock_num == 1 and Flg.Check("Battle5052") == false and round_count % 2 == 0 then
    ...
    if 0.2 <= enemy_now_hp / enemy_max_hp then
      ...
      Battle.SetExtraTurnNextCommandSkill(ENEMY_DIGIMON, 0, 29062)
    end
  elseif stock_num == 1 and Flg.Check("Battle5052") == false and round_count % 2 == 1 then
    Battle.SetExtraTurn(ENEMY_DIGIMON, 0, true)
    if 0.2 <= enemy_now_hp / enemy_max_hp then
      Battle.SetExtraTurnNextCommandSkill(ENEMY_DIGIMON, 0, 30042)
    else
      Battle.SetExtraTurnNextCommandSkill(ENEMY_DIGIMON, 0, 29062)
    end
  ...
  elseif stock_num == 2 and Flg.Check("Battle5052") == false and round_count % 2 == 1 then
    Battle.SetExtraTurn(ENEMY_DIGIMON, 0, true)
    if 0.2 <= enemy_now_hp / enemy_max_hp then
      Battle.SetExtraTurnNextCommandSkill(ENEMY_DIGIMON, 0, 30222)
    else
      Battle.SetExtraTurnNextCommandSkill(ENEMY_DIGIMON, 0, 29062)
    end
