...
function Battle_Command_Start(info)
  if Flg.Check("Battle5051") then
    Battle.SetNextCommandSkill(ENEMY_DIGIMON, 0, 24163)
...
function Battle_Turn_End(info)
  ...
  if Battle.IsAccomplishConditionType2(BATTLE_CONDITION_TYPE_DAMAGE_MAGNIFICATION_BELOW, 3, ENEMY_DIGIMON, 0, 4, 200, false) then
    Flg.Set("Battle5015")
  end
  if Battle.IsAccomplishCondition(BATTLE_CONDITION_TYPE_BUFF_COUNT_OVER, 0, ENEMY_DIGIMON, 0, 1) then
    Flg.Set("Battle5016")
  end
  if Battle.IsAccomplishCondition(BATTLE_CONDITION_TYPE_HIT_COUNT_OVER, 5, ENEMY_DIGIMON, 0, 12) then
    Flg.Set("Battle5017")
  end
  if Battle.GetCharacterLastCommandSkillID(ENEMY_DIGIMON, 0) == 24163 then
