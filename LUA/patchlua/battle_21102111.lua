...
function Battle_Command_Start(info)
  if Flg.Check("Battle5051") then
    Battle.SetNextCommandSkill(ENEMY_DIGIMON, 0, 24173)
...
function Battle_Turn_End(info)
  ...
  if Battle.IsAccomplishConditionType2(BATTLE_CONDITION_TYPE_DAMAGE_MAGNIFICATION_BELOW, 3, ENEMY_DIGIMON, 0, 2, 200, true) then
    Flg.Set("Battle5015")
  end
  local total_damage_num = Battle.GetStatus(ENEMY_DIGIMON, 0, STATUS_TYPE_HP_MAX) * 0.13
  ...
  if Battle.GetCharacterLastCommandSkillID(ENEMY_DIGIMON, 0) == 24173 then
...
function Battle_Ending(info)
  Common.SetDigimonGraspState(417, DIGIMON_GRASP_FLAG_SCAN)
end
