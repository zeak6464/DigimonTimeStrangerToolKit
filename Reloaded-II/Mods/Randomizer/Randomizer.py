...
def get_mod_base_path() -> Path:
    """Get the base path for the mod, handling both script and PyInstaller executable modes"""
    if getattr(sys, 'frozen', False):
        # Running as a PyInstaller executable
        if hasattr(sys, '_MEIPASS'):
            # Onefile mode: sys._MEIPASS is the temp extraction folder
            # sys.executable is the .exe location (where it's installed/permanently located)
            executable_path = Path(sys.executable)
            # Get the directory where the executable is installed (not the temp extraction folder)
            base_path = executable_path.parent
        else:
            # Onedir mode: sys.executable points to the executable in the bundle folder
            executable_path = Path(sys.executable)
            base_path = executable_path.parent
    else:
        # Running as a script
        # __file__ is the script location
        base_path = Path(__file__).parent
    
    # Returns the folder containing the script/executable (works regardless of folder name)
    # Expected structure: ModFolder/dsts-loader/app_0/data and ModFolder/data-backup
    return base_path
...
class RandomizerThread(QThread):
    ...
    def collect_all_column1_values(self) -> List[str]:
        ...
        # data_path is ModFolder/dsts-loader/app_0/data, so go up 3 levels to mod root
...
class FieldBattleRandomizerGUI(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        # Get the mod base path (handles both script and PyInstaller executable modes)
        mod_base = get_mod_base_path()
        
        # Point to the dsts-loader/app_0/data folder inside the mod folder
        # Field battle files go in app_0/data, other files stay in patch/data
        # Works regardless of what the mod folder is named
        self.data_path = mod_base / "dsts-loader" / "app_0" / "data"
        self.patch_data_path = mod_base / "dsts-loader" / "patch" / "data"
