...
-- Map chr IDs to character model names based on 000_char_info.csv
-- This is the source of truth for all available Digimon
local CHR_TO_CHAR = {
  ...
  chr563 = "char_ELECMON_AEGIOMON", chr564 = "char_ELECMON", chr567 = "char_PAGUMON",
  ...
  chr821 = "char_DUFTMON_BIG", chr822 = "char_JESMON_BIG", chr823 = "char_VULCANUSMON_BIG",
  ...
  chr930 = "char_CALAMARAMON_BIG", chr931 = "char_APOLLOMON_BIG", chr932 = "char_CERESMON_BIG",
  chr933 = "char_PLUTOMON_BIG", chr934 = "char_JUPITERMON_BIG", chr935 = "char_JUPITERMON_WRATHMODE_BIG",
  chr936 = "char_METALGREYMON_BIG_BL", chr937 = "char_TITAMON_BIG", chr938 = "char_CYCLONEMON_BIG",
...
function GetRandomDigimon(count)
  ...
  for i = 1, count do
    local index
    repeat
      index = math.random(1, #ALL_DIGIMON)
    until not used[index]
    
    used[index] = true
    table.insert(selected, ALL_DIGIMON[index])
  end
...
function GetCharacterName(chrID)
  ...
  local result = CHR_TO_CHAR[chrID] or "char_UNKNOWN"
...
-- Custom Digimon Configuration
-- Set USE_CUSTOM_DIGIMON to true and specify 3 chr IDs to use custom Digimon
-- Set USE_CUSTOM_DIGIMON to false or nil to use random selection
-- Example: CUSTOM_DIGIMON = {"chr050", "chr151", "chr114"}  -- Agumon, Gabumon, V-Mon
function GetCustomDigimon()
  local USE_CUSTOM_DIGIMON = false  -- Change to true to enable custom Digimon
  local CUSTOM_DIGIMON = {}  -- Set to 3 chr IDs, e.g., {"chr050", "chr151", "chr114"}
  
  if USE_CUSTOM_DIGIMON and CUSTOM_DIGIMON and #CUSTOM_DIGIMON == 3 then
    -- Validate that all custom Digimon exist in CHR_TO_CHAR (source of truth)
    local isValid = true
    for i, digimon in ipairs(CUSTOM_DIGIMON) do
      if not CHR_TO_CHAR[digimon] then
        print("WARNING: Invalid custom Digimon at index " .. i .. ": " .. digimon)
        isValid = false
      end
    end
    
    if isValid then
      print("=== USING CUSTOM DIGIMON ===")
      return CUSTOM_DIGIMON
    else
      print("=== CUSTOM DIGIMON INVALID, FALLING BACK TO RANDOM ===")
      return nil
    end
  end
...
function m010_090()
  -- Check for custom Digimon first, then fall back to random
  local customDigimon = GetCustomDigimon()
  if customDigimon then
    RANDOM_DIGIMON = customDigimon
  else
...
function Load_PDigiSelectHologram()
  ...
  Vista.LoadAnimation("DECIDE_" .. RANDOM_DIGIMON[1], "bn01")
  Vista.LoadAnimation("DECIDE_" .. RANDOM_DIGIMON[2], "bn01")
  Vista.LoadAnimation("DECIDE_" .. RANDOM_DIGIMON[3], "bn01")
end
