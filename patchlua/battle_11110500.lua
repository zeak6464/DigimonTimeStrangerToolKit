...
function Battle_Turn_Start(info)
  ...
  if is_high_difficulty == true and Flg.Check("Battle5103") == true and Battle.GetStatus(GUEST_DIGIMON, 0, STATUS_TYPE_LV) < 99 then
    Battle.SetStatus(GUEST_DIGIMON, 0, STATUS_TYPE_LV, 99)
  end
  if Flg.Check("Battle5053") then
    ...
    if info.group == 2 and info.index == 0 then
      ...
      if Flg.Check("Battle5103") then
        Battle.SetNextCommandSkill(GUEST_DIGIMON, 0, 30193)
      end
    end
  end
...
function Battle_Command_Start(info)
  ...
  if ChargeRound == 2 and Flg.Check("Battle5053") and enemy_id == 49201 then
...
function Battle_Action_Start(info)
  ...
  if info.group == GUEST_DIGIMON and info.digimon_id == 174 then
    local skill_id = Battle.GetLastCommandSkillID()
    if (skill_id == 21741 or skill_id == 21742) and info.use_count == 0 then
      Battle.SetBattleSpeedToNormalForTurn()
    end
  elseif info.group == ENEMY_DIGIMON and info.enemy_id == 49201 and Battle.GetLastCommandSkillID() == 24923 and info.use_count == 0 then
    Battle.SetBattleSpeedToNormalForTurn()
  end
end
