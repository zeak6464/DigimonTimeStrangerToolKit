...
function LoadRequest()
  ...
  elseif gMapNum == 103 then
    ...
    local npc_0003 = GetIndex(NPC, "npc_0003")
    Npc(npc_0001).LoadMotion("ft01_02")
    Npc(npc_0002).LoadMotion("ft01_02")
    Npc(npc_0003).LoadMotion("ft01_01")
    ...
    Npc(douger_1).LoadMoveMotion("")
    Npc(douger_2).LoadMoveMotion("")
    Npc(douger_3).LoadMoveMotion("")
    Npc(douger_4).LoadMoveMotion("")
    Npc(douger_5).LoadMoveMotion("")
    Npc(douger_move).LoadMoveMotion("")
    CtrlLoadNPCMotion("douger_move", "ft01_01", "ft01_02", "ft02_01", "ft02_02", "e109", "e111")
  ...
  elseif gMapNum == 103 then
    ...
    if FlagCheck(FLAG_MAIN_03_260) and FlagCheck(FLAG_GIMMICK_T01_020) then
      ...
      if player_move_state == "move" then
        if total_frame <= 130 then
          SetInstantCameraInNowFrame(cam_aim_x, 1.6, cam_aim_z - 1, cam_pos_x, 2.5, cam_pos_z + 2.5, 70, 60, 120, QUINTIC_OUT, QUINTIC_OUT)
        elseif 130 < total_frame then
          SetKeyInstantCameraInNowFrame("1", cam_aim_x, 1.6, cam_aim_z - 1, cam_pos_x, 2.5, cam_pos_z + 2.5, 70, frame, 120, QUINTIC_OUT, QUINTIC_OUT)
      ...
      elseif player_move_state == "lastcamera" then
        ...
        if total_frame_2 <= 30 then
          ...
          Player().SetRailMoveMaximumLeftAndRightDistanceAwayFromTheRail(0.1)
          Player().RailMoveGo()
        elseif 30 < total_frame_2 and total_frame_2 <= 91 then
    ...
    if FlagCheck(FLAG_MAIN_01_030) and not FlagCheck(FLAG_MAIN_01_060) then
      ...
      if FlagCheck(FLAG_GIMMICK_T01_070) then
        ...
        if player_move_state == "move" then
          ...
          if IsPadTrigger(PAD_INPUT_L2) == false and IsPadTrigger(PAD_INPUT_R2) == false and operatedegrees ~= nil and 45 < total_frame and (operatedegrees <= 120 or 240 <= operatedegrees) then
