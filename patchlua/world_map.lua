function OpenWorldMap(field_type, transportation)
  ...
  while not WorldMap.IsSelected() and not WorldMap.IsClosed() and not WorldMap.IsCanceled() do
    WaitFrame(1)
  end
  if WorldMap.IsCanceled() then
    FadeOutWithWaitNotFlgCheck(FADE_BLACK, 5)
    WorldMap.Close()
    while true do
    ...
    if prefix == "d" then
      ...
      if map_num == 606 then
        ...
        if Flg.Check("FLAG_MAIN_07_000", "FLAG_MAIN_07_010") then
          ...
          tlk:StartTalk()
          if not Flg.Check("FLAG_IS_CLEAR_SUB_002") then
