...
function Battle_Round_Start(info)
  ...
  if round_count % 4 == 0 and 0 < Battle.GetStatus(ENEMY_DIGIMON, 0, STATUS_TYPE_HP) then
...
function Battle_Turn_Start(info)
  ...
  if Flg.Check("Battle5004") or Flg.Check("Battle5005") then
    Talk.OpenLetterBox()
    Battle.CloseUI()
    WaitFrame(1)
    ...
    Battle.SetModelPosition(ENEMY_DIGIMON, 0, -4.94, 0, -5.6)
    ...
    if Flg.Check("Battle5003") == false then
      Battle.SetOverwriteCamera("camera_big_boss07")
      Flg.Set("Battle5003")
    end
  end
end
...
function Battle_Turn_End(info)
  ...
  local total_damage_num = Battle.GetStatus(ENEMY_DIGIMON, 0, STATUS_TYPE_HP_MAX) * 0.18
