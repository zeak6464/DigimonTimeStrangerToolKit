...
function Update(elapsed_frame)
  if gMapNum == 701 then
    ...
    if FlagCheck(FLAG_MAIN_20_076) and not FlagCheck(FLAG_MAIN_20_080) then
      ...
      local speed = GetLeftAxisPlayerMoveMaxSpeed(false) * GetLeftAxisPlayerMoveOperateRate()
      if GetExternalVariable("Cam_M420_100") == "cam_01" then
        speed = speed * 0.2
      ...
      elseif GetExternalVariable("PlayVoice_M420_100_01") == "stop" and IsArrivalPosition(NPC, npc_aigio02, PLAYER, 1, 5) then
        Player().Move_Stop()
      end
      ...
      elseif GetExternalVariable("PlayVoice_M420_100_02") == "playing" then
        if IsArrivalPosition(NPC, npc_aigio03, PLAYER, 1, 5.3) then
          Player().Move_Stop()
        end
        if IsPlayingVoiceOver() == false then
          SetExternalVariable("PlayVoice_M420_100_02", "stop")
        end
      elseif GetExternalVariable("PlayVoice_M420_100_02") == "stop" and IsArrivalPosition(NPC, npc_aigio03, PLAYER, 1, 5.3) then
        Player().Move_Stop()
      end
      ...
      elseif GetExternalVariable("PlayVoice_M420_100_03") == "playing" then
        if IsArrivalPosition(NPC, npc_aigio04, PLAYER, 1, 5.3) then
          Player().Move_Stop()
        end
        if IsPlayingVoiceOver() == false then
          SetExternalVariable("PlayVoice_M420_100_03", "stop")
        end
      elseif GetExternalVariable("PlayVoice_M420_100_03") == "stop" and IsArrivalPosition(NPC, npc_aigio04, PLAYER, 1, 5.3) then
        Player().Move_Stop()
      end
    end
  end
